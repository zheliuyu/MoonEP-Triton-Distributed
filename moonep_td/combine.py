...
def _kernel():
    ...
    def combine_kernel(
        ...
        if token >= S:
            return
        stride_out_h64 = stride_out_h.to(tl.int64)
        stride_buf_row64 = stride_buf_row.to(tl.int64)
        dst_row = output_ptr + token.to(tl.int64) * stride_out_h64
        for h_off in range(0, H, BLOCK_H):
            ...
            for k in range(K):
                ...
                if dst_val >= 0:
                    ...
                    src_row = remote_buf + row_idx * stride_buf_row64
...
def launch_combine(
    ...
    *,
    ...
    if output_sk is not None:
        assert output_sk.dtype == torch.float32 and output_sk.is_contiguous(), "output_sk must be contiguous fp32"
    ...
    combine_kernel[(S,)](
        ...
        output_sk,
        ctx["hidden_buf"],
        ctx["meta_buf"].view(torch.float32),
        dst,
        output_sh.stride(0),
