...
from tests.kernel_test_utils import (
    ...
    grad_reduce_kwargs,
    make_grad_reduce_nvl_buffers,
    release_grad_reduce_nvl_tensors,
)
...
def make_remote_expert(rank, R, E, H, Hp, owner_offset=1):
    ...
    for owner in range(R):
        ...
        if rank == owner:
            ...
            mapped[:E].copy_(torch.randn(E, H, Hp, dtype=torch.bfloat16, device=dev, generator=g))
            if padded_E > E:
                mapped[E:].zero_()
...
def assert_prefetch_slots_equal(prefetch_args, B, value):
    for name, full_weight in prefetch_args.items():
        ...
        assert torch.equal(full_weight[-B:], expected), f"{name} prefetch slots changed without prefetch_weight"
...
def make_grad_reduce_args(rank, R, E, B, H, Hp, offsets):
    ...
    args.update(make_grad_reduce_nvl_buffers(rank, R, B, H, Hp, offsets, reduce_base))
    return args
...
def assert_grad_reduced(rank, R, E, H, Hp, experts_to_copy, args, offsets):
    ...
    for name, full_grad, reduce_buffer, full_offset, reduce_offset in pairs:
        expected = expected_local_grad(rank, R, E, H, Hp, full_offset, reduce_offset, experts_to_copy)
        assert torch.allclose(full_grad[local_start:local_end], expected, rtol=0.0, atol=1e-5), (
        ...
        original_reduce = reduce_base(R, experts_to_copy.shape[1], H, Hp, reduce_offset, reduce_buffer.device)
        for src_rank in range(R):
            for b in range(experts_to_copy.shape[1]):
                expert = int(experts_to_copy[src_rank, b].item())
...
def assert_raises_assertion(expected_substr, fn):
    try:
        ...
        assert expected_substr in str(exc), f"expected assertion containing {expected_substr!r}, got {exc!r}"
    else:
...
def test_e2e(dist_env):
    ...
    for name in ("dst", "experts_to_copy", "zero_fill_ranges", "remote_stats"):
        assert torch.equal(getattr(plan_snapshot, name), getattr(plan_a, name)), f"plan.{name} mismatch"
    ...
    torch.cuda.synchronize()
    assert torch.equal(h_no_prefetch_async.clone(), h_sync_cpu), "async no-prefetch hidden mismatch"
    ...
    assert torch.equal(h_zc, h_for_combine), "zero_copy dispatch hidden view mismatch vs copied tensor"
    assert torch.equal(w_zc, w_for_combine), "zero_copy dispatch weights view mismatch vs copied tensor"
    ...
    assert torch.equal(gathered_weights_zc, weights), "zero_copy combine route_weights_sk gather mismatch"
    ...
    buffer.reduce_grad(plan=plan_snapshot, **grad_reduce_kwargs(grad_args))
    release_grad_reduce_nvl_tensors(grad_args)
    torch.cuda.synchronize()
    ...
    reduce_ev = buffer.reduce_grad(
        plan=plan_snapshot,
        async_finish=True,
        **grad_reduce_kwargs(async_grad_args),
    )
    reduce_ev.wait(torch.cuda.current_stream())
    release_grad_reduce_nvl_tensors(async_grad_args)
    torch.cuda.synchronize()

    assert torch.equal(out_sync_snap, out_async), "combine output mismatch"
    assert_grad_reduced(rank, R, E, H, Hp, plan_snapshot.experts_to_copy, async_grad_args, async_grad_offsets)
